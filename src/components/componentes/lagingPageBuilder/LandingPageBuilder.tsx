      ...
      <main className="flex-grow p-4">
        {containers.map((container, containerIndex) => (
          <ContainerComponent
            ...
            onUpdateComponent={(componentIndex, newProps) => updateComponent(containerIndex, componentIndex, newProps)}
            onRemoveComponent={(componentIndex) => removeComponent(containerIndex, componentIndex)}
            onUpdateConfig={(newConfig) => updateContainerConfig(containerIndex, newConfig)}
...
function ContainerComponent({ container, containerIndex, onAddComponent, onUpdateComponent, onRemoveComponent, onUpdateConfig, onAddSubContainer }: { container: Container, containerIndex: number, onAddComponent: (type: ComponentType) => void, onUpdateComponent: (componentIndex: number, newProps: any) => void, onRemoveComponent: (componentIndex: number) => void, onUpdateConfig: (newConfig: Partial<ContainerConfig>) => void, onAddSubContainer: () => void }) {
  return (
    ...
    >
      ...
      <div className={`flex ${container.config.alignment} ${container.config.justifyContent} ${container.config.alignItems} gap-4`}>
        {container.components.map((component, componentIndex) => (
          <div key={componentIndex} className="relative w-full">
            ...
            {component.type === 'container' ? (
              ...
              <ComponentRenderer
                component={component}
                onUpdate={(newProps) => onUpdateComponent(componentIndex, newProps)}
              />
            )}
          </div>
        ))}
      ...
      <nav>
        {config.navItems.map((item, index) => (
          item.subItems ? (
            ...
            <Popover key={index}>
              ...
              <PopoverContent>
                {item.subItems.map((subItem, subIndex) => (
                  <Button key={subIndex} variant="ghost" className="w-full justify-start">
                    {subItem.label}
                  </Button>
                ))}
              </PopoverContent>
            ...
            <Button key={index} variant="ghost" className="mx-1">{item.label}</Button>
          )
        ))}
      </nav>
      <Dialog>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {config.navItems.map((item, index) => (
                <div key={index} className="flex items-center space-x-2 mt-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const newNavItems = [...config.navItems]
                      newNavItems[index] = { ...newNavItems[index], href: e.target.value }
                      onConfigUpdate({ ...config, navItems: newNavItems })
                    }}
                    placeholder="Link"
                  />
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      const newNavItems = [...config.navItems]
                      newNavItems[index] = { ...newNavItems[index], isList: checked }
                      onConfigUpdate({ ...config, navItems: newNavItems })
                    }}
                  />
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={() => {
                      const newNavItems = config.navItems.filter((_, i) => i !== index)
                      onConfigUpdate({ ...config, navItems: newNavItems })
                    }}
                  >
                    <X className="h-4 w-4" />
              ...
              <Button
                onClick={() => onConfigUpdate({ ...config, navItems: [...config.navItems, { label: 'New Item', href: '#' }] })}
                className="mt-2"
              >
                Add Nav Item
              </Button>
...
function AddComponentButton({ onAddComponent, disabled, onAddSubContainer }: { onAddComponent: (type: ComponentType) => void; disabled: boolean; onAddSubContainer: () => void }) {
  return (
    <Dialog>
      <DialogTrigger asChild>
      ...
      <DialogContent>
        ...
        <div className="grid grid-cols-2 gap-4">
          {(['paragraph', 'button', 'image', 'video', 'carousel', 'form', 'table'] as ComponentType[]).map((type) => (
            <Button key={type} onClick={() => onAddComponent(type)} className="h-20 text-lg capitalize">
              {type}
            </Button>
          ))}
          <Button onClick={onAddSubContainer} className="h-20 text-lg capitalize">
            Sub-Container
          </Button>
        </div>
      </DialogContent>
...
function ParagraphComponent({ content, font, fontSize, alignment, bold, italic, underline, textColor, backgroundColor, padding, margin, onUpdate }: { content: string; font: string; fontSize: number; alignment: string; bold: boolean; italic: boolean; underline: boolean; textColor: string; backgroundColor: string; padding: string; margin: string; onUpdate: (newProps: any) => void }) {
  return (
...
function ButtonComponent({ text, link, font, fontSize, alignment, bold, italic, underline, textColor, backgroundColor, padding, margin, width, onUpdate }: { text: string; link: string; font: string; fontSize: number; alignment: string; bold: boolean; italic: boolean; underline: boolean; textColor: string; backgroundColor: string; padding: string; margin: string; width: string; onUpdate: (newProps: any) => void }) {
  return (
    <div className="space-y-2">
      <Input
        value={text}
...
function ImageComponent({ src, alt, width, height, opacity, objectFit, onUpdate }: { src: string; alt: string; width: number; height: number; opacity: number; objectFit: string; onUpdate: (newProps: any) => void }) {
  ...
  return (
    <div className="space-y-2">
      <img src={src} alt={alt} className={`max-w-full h-auto mb-2 ${objectFit}`} style={{ width, height, opacity: opacity / 100 }} />
      <Input
        type="file"
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select onValueChange={(value) => onUpdate({ objectFit: value })} value={objectFit}>
          <SelectTrigger className="w-[180px]">
...
function VideoComponent({ src, width, height, autoplay, muted, defaultSize, controls, onUpdate }: { src: string; width: number; height: number; autoplay: boolean; muted: boolean; defaultSize: boolean; controls: boolean; onUpdate: (newProps: any) => void }) {
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) {
      const reader = new FileReader()
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Input
          type="number"
          value={visibleSlides}
          onChange={(e) => onUpdate({ visibleSlides: parseInt(e.target.value) })}
          className="w-20"
...
function FormComponent({ title, subtitle, font, fontSize, alignment, fields, buttonText, buttonColor, buttonTextColor, onUpdate }: { title: string; subtitle: string; font: string; fontSize: number; alignment: string; fields: { label: string; type: string }[]; buttonText: string; buttonColor: string; buttonTextColor: string; onUpdate: (newProps: any) => void }) {
  const addField = () => {
...
function TableComponent({ rows, columns, headers, data, striped, hoverable, bordered, onUpdate }: { rows: number; columns: number; headers: string[]; data: string[][]; striped: boolean; hoverable: boolean; bordered: boolean; onUpdate: (newProps: any) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Table className={`${striped ? 'table-striped' : ''} ${hoverable ? 'table-hover' : ''} ${bordered ? 'table-bordered' : ''}`}>
        <TableHeader>
          <TableRow>
...
function PreviewParagraph({ content, font, fontSize, alignment, bold, italic, underline, textColor, backgroundColor, padding, margin }: { content: string; font: string; fontSize: number; alignment: string; bold: boolean; italic: boolean; underline: boolean; textColor: string; backgroundColor: string; padding: string; margin: string }) {
  const style = {
    fontFamily: font,
...
function PreviewButton({ text, link, font, fontSize, alignment, bold, italic, underline, textColor, backgroundColor, padding, margin, width }: { text: string; link: string; font: string; fontSize: number; alignment: string; bold: boolean; italic: boolean; underline: boolean; textColor: string; backgroundColor: string; padding: string; margin: string; width: string }) {
  const style = {
    fontFamily: font,
    fontSize: `${fontSize}px`,
    textAlign: alignment as any,
...
function PreviewVideo({ src, width, height, autoplay, muted, defaultSize, controls }: { src: string; width: number; height: number; autoplay: boolean; muted: boolean; defaultSize: boolean; controls: boolean }) {
  return (
    <video
      controls={controls}
      src={src}
...
function PreviewCarousel({ items, slideCount, showText, showLinks, showButtons, isCardType, autoPlay, interval, visibleSlides }: { items: any[]; slideCount: number; showText: boolean; showLinks: boolean; showButtons: boolean; isCardType: boolean; autoPlay: boolean; interval: number; visibleSlides: number }) {
  return (
    <Carousel className="w-full max-w-xs mx-auto">
      <CarouselContent>
        {items.slice(0, slideCount).map((item, index) => (
...
function PreviewForm({ title, subtitle, font, fontSize, alignment, fields, buttonText, buttonColor, buttonTextColor }: { title: string; subtitle: string; font: string; fontSize: number; alignment: string; fields: { label: string; type: string }[]; buttonText: string; buttonColor: string; buttonTextColor: string }) {
  return (
    <form className={`${font} text-${fontSize}px ${alignment} space-y-4`}>
      ...
      <Button type="submit" className={`${buttonColor} ${buttonTextColor}`}>{buttonText}</Button>
    </form>
  )
}

function PreviewTable({ rows, columns, headers, data, striped, hoverable, bordered }: { rows: number; columns: number; headers: string[]; data: string[][]; striped: boolean; hoverable: boolean; bordered: boolean }) {
  return (
    <Table className={`${striped ? 'table-striped' : ''} ${hoverable ? 'table-hover' : ''} ${bordered ? 'table-bordered' : ''}`}>
      <TableHeader>
        <TableRow>
...
function PreviewContainer({ components, config }: { components: Component[]; config: ContainerConfig }) {
  return (
    ...
    >
      <div className={`flex ${config.alignment} ${config.justifyContent} ${config.alignItems} gap-4`}>
